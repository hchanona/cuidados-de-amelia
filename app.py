...
with st.form("registro"):
    ...
    leche_container = st.empty()
    tipo_leche_container = st.empty()
    popo_container = st.empty()
    ...
    if tipo == "toma de leche":
        cantidad_leche_oz = leche_container.number_input("Cantidad de leche (oz)", min_value=0.0, step=0.1)
        cantidad_leche_ml = cantidad_leche_oz * 29.5735
        tipo_leche = tipo_leche_container.selectbox("Tipo de leche", ["materna", "Puramino"])
    elif tipo == "puenteo":
        cantidad_popo_puenteada = popo_container.number_input("Cantidad de popó puenteada (ml)", min_value=0, step=1)
