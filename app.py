...
# === Hora actual CDMX (ajuste manual UTC-6) ===
...
if not data.empty:
    ...
    data = data[data["fecha_hora"] <= ahora]  # Ignorar eventos futuros
...
with st.form("registro"):
    ...
    # Render condicional de campos según tipo
...
if not data.empty:
    ...
    if dias_desde_cambio is not None:
        horas = int((ahora - ultima_colocacion).total_seconds() // 3600)
        minutos = int((ahora - ultima_colocacion).total_seconds() % 3600 // 60)
        st.metric("🩹 Desde última colocación de bolsa", f"{horas} h {minutos} min")
